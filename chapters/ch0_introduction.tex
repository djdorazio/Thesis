...
\subsection{The gravitational side}  %Gravitational radiation is generated
when the shape of a mass-energy %distribution changes in time. More precisely,
when quadrapole and higher %moments of a mass-energy distribution have a
changing current, metric perturbations % Gravitational radiation is generated
...
%at angular frequency $\omega$ has
%rotational energy $1/2 M r^2 \omega^2$}
...
scattering for photons -- the cosmic microwave background!}. %, in which case
...
ArmNat:2002:ApJL}. %Recent observations of MBHB candidates at sub-pc separtion
%seem to suggest that the FPP is not really a problem after all. 
...
Sesana+2016:GWB, Arzoumanian:2015:SGWB, PTAsGWB}. In addition to its importance for orbital dynamics,
...
%Gas cool into thin disk:  Escala et al. 2005
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% FIGURE: 2D density%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}
...
groundwork for disk interactions with very small mass ratio systems, where the
response of the binary and disk can be explored with linear perturbation
analysis. In this case, the secondary launches linear spiral density waves
from the locations of Linblad resonances in the disk
\citep{LyndenBellKalnajs:1972}. Summing contributions from torques exerted on
the disk at these resonances, \cite{GT80} were the first to show that the
back-reaction of the disk perturbations onto the binary cause the binary
orbital separation to change. For disks with keplerian rotation profiles,
...
\citep{GT80, Ward:1988, GoldreichSari:2003}. This steady spiral density waves solution is depicted in the left panel of Figure \ref{Fig:IntroHydro}.
...
% much more violent clearing of a \emph{central cavity} for mass ratios above $q
% \sim0.04$.
...
%The calculation for NSBH systems, 
...
\subsection{An analogy form Faraday}
...
\citep{MPBook}, then we can calculate the emf generated by the NSBH system.

In the NSBH system, currents are carried by electrons and positrons spiraling
...
when $R_H$ is the horizon radius. %There an infinite number of these circuits
at all times.
...
NSNS binaries. From such observations we could learn about the NS magnetic fields strengths at merger, the NS equation of state, and the dynamics of high energy EM fields in curved spacetime. Stay Tuned.
